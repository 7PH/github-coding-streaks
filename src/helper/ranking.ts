...
export function computeDayStreak({ user, allContributionDays }: EnhancedGithubUser) {
...
export function computeWeekStreak({ user, allContributionWeeks }: EnhancedGithubUser) {
    const getWeekCount = (week: ContributionWeek) =>
        week.contributionDays.map((day) => day.contributionCount).reduce((a, b) => a + b, 0);

    // Get previous sunday using moment()
    const today = new Date();
